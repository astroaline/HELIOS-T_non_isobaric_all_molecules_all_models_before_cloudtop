...
class Model:
    ...
    def transit_depth(self):
        ...
        if self.parameter_dict["log_P_cloudtop"] == "Off":
            cloud_tau = np.zeros((1, len(pressure_values)))
        else:
            cloudtop_pressure = (10**self.parameter_dict["log_P_cloudtop"])*1e6
            cloud_tau = np.zeros((1,len(pressure_values)))
            cloud_tau[0,pressure_values > cloudtop_pressure] = np.inf


        tau_values = factor * (integral_molecules_sum + integral_grid_cia*xh2 + integral_grid_rayleigh*xh2) + cloud_tau.T
